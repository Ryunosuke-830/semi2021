...
class HumanPoseTeleop:
    ...
    def callback(self, msg):
        ...
        if msg.poses:
            ...
            tw = Twist()
            if right_wrist_pos is None:
                tw.linear.y = 0.0
                tw.linear.z = 0.0
            else:
                controlY = -(right_wrist_pos.x - baseX)
                controlZ = -(right_wrist_pos.y - baseY)*2.0
                tw.linear.y = controlY/width
                tw.linear.z = controlZ/height
                #str1 = "right: x: {}, y: {} ".format(controlY,controlZ)
                #str2 = "left: wristY={}, shoulderY={}".format(left_wrist_pos.y, left_shoulder_pos.y)
                #rospy.loginfo_throttle(0.5, str1)
            
            if  left_wrist_pos is None:
                tw.linear.x = 0.0
            else:
                controlX = -(left_wrist_pos.y - baseY)
                tw.linear.x = controlX/height
